...
when "ubuntu", "debian"
  ...
  execute "curl -s http://pkg.hudson-labs.org/debian/hudson-labs.org.key | apt-key add -" do
    not_if "apt-key export 'Hudson Apt Repository'"
  end
...
user node[:hudson][:server][:user] do
  comment "Hudson"
  gid node[:hudson][:user]
  home node[:hudson][:home]
  shell "/bin/sh"
end
